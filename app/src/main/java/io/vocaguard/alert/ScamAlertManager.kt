...
class ScamAlertManager(private val context: Context) {
    ...
    fun triggerScamAlert(
        ...
        // Respect per-scam-type suppression before doing anything.
        if (!settings.isAlertEnabledFor(scamType)) {
            Log.d(TAG, "Alert suppressed for $scamType (disabled in settings)")
            return
        }

        Log.w(TAG, "Triggering scam alert: $scamType (confidence: ${confidence * 100}%)")

        if (settings.enableSound) playWarningSound()
        if (settings.enableVibration) vibrateWarning()
        if (settings.enableTts) speakAlert(scamType)
        showAlertNotification(scamType, transcript, confidence, phoneNumber)

        // Notify family members / caregivers via SMS and/or webhook (fire-and-forget).
        alertScope.launch {
