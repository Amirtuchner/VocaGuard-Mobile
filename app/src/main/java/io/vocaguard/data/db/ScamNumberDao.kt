...
interface ScamNumberDao {
    ...
    /** Removes entries whose [ScamNumberEntity.expiresAt] is non-zero and in the past. */
    @Query("DELETE FROM scam_numbers WHERE expiresAt != 0 AND expiresAt < :nowMs")
    suspend fun deleteExpiredBefore(nowMs: Long)
}
